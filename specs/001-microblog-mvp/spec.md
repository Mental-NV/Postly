...
- **FR-072**: The build pipeline MUST synchronize the frontend build output into
  the backend `wwwroot` during the `Postly.Api` pre-build stage so the backend
  remains the single runtime entry point for the full application. The expected
  mechanism is an MSBuild target equivalent to `SyncSpaAssetsToWwwroot` that
  copies frontend `dist` assets into `wwwroot` for non-Release local builds.
- **FR-073**: Running `dotnet run --project backend/src/Postly.Api` MUST start the full MVP
  ...
  migrations or equivalent startup preparation applied, without requiring a
  separate manual frontend start command.
- **FR-074**: End-to-end Playwright execution MUST target the application
  started through `dotnet run --project backend/src/Postly.Api` rather than a separate
  standalone frontend dev server, and MUST use a best-effort readiness check
  that waits for the backend-served SPA entry point and configured `DataSeed`
  preparation before browser interactions begin.
- **FR-075**: Publish output MUST include the frontend static assets under
  `wwwroot` through an MSBuild publish-stage target equivalent to
  `IncludeSpaDistInPublish`, wired before publish file computation so deployable
  output includes the SPA without requiring a separate manual copy step.
- **FR-076**: Repository ignore rules MUST exclude local SQLite database files,
...
- **FR-077**: Protected-access, empty, error, and not-available messages MUST
  state the current outcome and present at least one explicit next action such
  as sign in, retry, create a post, follow users, or return home.
- **FR-078**: Repository ignore rules MUST treat local runtime, development,
  and test SQLite database files as non-versioned artifacts; no generated local
  seed or runtime DB file is to be committed unless a separate sanitized sample
  artifact is explicitly introduced and documented.
